...
| `sparrow mcp [-s profile]` | serve **orient / sql / pull / expect / verify**, the diagnostics (**doctor / check / ping**, plus `sql` stats-benchmark mode) and client-side **version / whatsnew / feedback** over **MCP stdio** — chat agents without a shell (Claude Desktop, claude.ai) drive the bound Flight server through schema-validated tool calls on ONE warm connection | dialed lazily, kept warm |
...
verify** as MCP tools (stdio) against the one server it's bound to — plus the
diagnostic layer (**doctor** with its conformance card, **check**, **ping**,
and `sql`'s stats-benchmark mode) and **version**, **whatsnew** (release
notes, live from the feed), and **feedback** (file a bug/idea to the
maintainers; needs no Flight server at all — reports name the MCP client
that filed them). What the
